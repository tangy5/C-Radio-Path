...
def _make_ext(ext_cls, name, module, sources, include_dirs=None, define_macros=None, extra_flags=None):
    """Build a C++ extension (CppExtension or CUDAExtension).

    Args:
        ext_cls: Extension class (CppExtension or CUDAExtension).
        name (str): Name of the op.
        module (str): Name of the module with the op.
        sources (list): List of source files.
        include_dirs (list): List of include directories relative to the module.
        define_macros (list): List of (name, value) macro definitions.
        extra_flags (dict): Any extra compile flags.

    Returns:
        Extension object for setup().
    """
...
def make_cpp_ext(name, module, sources, include_dirs=None, define_macros=None, extra_flags=None):
    """Build C++ extensions for custom ops (no CUDA dependency).
    ...
    Returns:
        cpp_ext (torch.utils.cpp_extension.CppExtension): C++ extension for wheeling.
    """
    return _make_ext(CppExtension, name, module, sources, include_dirs, define_macros, extra_flags)
...
def make_cuda_ext(name, module, sources, include_dirs=None, define_macros=None, extra_flags=None):
    ...
    """
    return _make_ext(CUDAExtension, name, module, sources, include_dirs, define_macros, extra_flags)
