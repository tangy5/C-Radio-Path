...
class CustomModelCheckpoint(ModelCheckpoint):
    """Custom callback for saving NVDINOv2 checkpoint with best + periodic saving.

    Saves two types of checkpoints:
    1. Best model (monitored by a metric like distillation_loss)
    2. Periodic checkpoints every `every_n_epochs` (old ones auto-cleaned, keeps last N)

    This prevents disk exhaustion while preserving the most important checkpoints.
    """

    def __init__(self, keep_periodic: int = 3, **kwargs):
        """Args:
            keep_periodic: Number of periodic checkpoints to keep (older ones deleted).
        """
        super().__init__(**kwargs)
        self.keep_periodic = keep_periodic
        self._periodic_dir = os.path.join(self.dirpath, "periodic")
        os.makedirs(self._periodic_dir, exist_ok=True)
    ...
    def _save_checkpoint(self, trainer: "pl.Trainer", filepath: str) -> None:
        ...
        """
        # Save the full training checkpoint to periodic dir
        periodic_filepath = os.path.join(self._periodic_dir, os.path.basename(filepath))
        trainer.save_checkpoint(periodic_filepath, self.save_weights_only)

        # Also save to the standard path (for best/latest model tracking)
    ...
    def _cleanup_periodic_checkpoints(self):
        ...
        if not os.path.isdir(self._periodic_dir):
            return
        checkpoints = sorted(
            glob.glob(os.path.join(self._periodic_dir, f"*{self.FILE_EXTENSION}")),
            key=os.path.getmtime,
        )
        while len(checkpoints) > self.keep_periodic:
...
class DinoV2PlModel(TAOLightningModule):
    ...
    def configure_callbacks(self) -> Sequence[Callback] | pl.Callback:
        ...
        checkpoint_callback = CustomModelCheckpoint(
            ...
            monitor="distillation_loss_raw_epoch",
            mode="min",
            save_top_k=1,
            save_last="link",
