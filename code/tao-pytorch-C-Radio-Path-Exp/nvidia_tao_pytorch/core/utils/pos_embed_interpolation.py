...
def interpolate_pos_embed(checkpoint_model,
    ...
    if 'pos_embed' in checkpoint_model:
        ...
        # Auto-detect num_extra_tokens (e.g., cls_token)
        # Some checkpoints (e.g., DINOv2/H-optimus) store only patch tokens in pos_embed
        # without cls_token, so total tokens == perfect square means 0 extra tokens.
        num_tokens = pos_embed_checkpoint.shape[-2]
        grid_size_sq = int(num_tokens ** 0.5)
        if grid_size_sq * grid_size_sq == num_tokens:
            num_extra_tokens = 0
        else:
            num_extra_tokens = 1
