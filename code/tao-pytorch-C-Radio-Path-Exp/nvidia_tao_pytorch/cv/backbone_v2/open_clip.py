...
class OpenCLIP(BackboneBase):
    ...
    def get_stage_dict(self):
        ...
        """
        if isinstance(self.model.visual, timm_model.TimmModel):
            # TODO: Does TimmModel have a unified signature?
...
@BACKBONE_REGISTRY.register()
def vit_h_14_siglip_clipa_336(**kwargs):
    """Create a ViT Huge Patch14 SigLIP CLIPA 224 model.
    ...
    Example:
        >>> model = vit_h_14_siglip_clipa_336(num_classes=1000)
        >>> x = torch.randn(1, 3, 336, 336)
        >>> output = model(x)  # Shape: (1, 1000)
    ...
    Note:
        ...
        requires more computational resources.
    """
    return OpenCLIP(model_name="ViT-H-14-SigLIP-CLIPA-336", **kwargs)


@BACKBONE_REGISTRY.register()
def vit_h_14_siglip_clipa_574(**kwargs):
    """Create a ViT Huge Patch14 SigLIP CLIPA 224 model.

    This function creates a ViT Huge model with the following specifications:
    - Patch size: 14x14 pixels
    - Image size: 574x574 pixels
    - Embedding dimension: 1024
    - Number of layers: 32
    - Width: 1280
    - Head width: 80
    ...
    Example:
        >>> model = vit_h_14_siglip_clipa_574(num_classes=1000)
        >>> x = torch.randn(1, 3, 336, 336)
        >>> output = model(x)  # Shape: (1, 1000)
    ...
    Note:
        ...
        requires more computational resources.
    """
    return OpenCLIP(model_name="ViT-H-14-SigLIP-CLIPA-574", **kwargs)