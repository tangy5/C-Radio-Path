...
class RADIO(BackboneBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # TODO: This is actually a redundant wrapper for the RADIO models. We can remove it in the future.
...
@BACKBONE_REGISTRY.register()
def c_radio_v4_so400m_patch16(**kwargs):
    """CRADIOV4 SO400M Patch16.
    
    C-RADIOv4 model with Shape-Optimized 400M architecture.
    Architecture: vit_so400m_patch16_224
    - Embedding dim: 1152
    - Depth: 27 layers
    - Heads: 16
    - Parameters: ~431M
    
    Pretrained with teachers: SigLIP2-g, DINOv3-7B, SAM3
    """
    return RADIO(
...
def c_radio_v4_vit_huge_patch16(**kwargs):
    ...
    C-RADIOv4 model with ViT-Huge architecture.
    Architecture: vit_huge_patch16_224 (NO LayerScale, matches pretrained checkpoint)
    - Embedding dim: 1280
    - Depth: 32 layers
    - Heads: 16
    - Parameters: ~653M
    ...
    Pretrained with teachers: SigLIP2-g, DINOv3-7B, SAM3
    Uses num_teacher=3, register_multiple=10 to match
    the C-RADIOv4-H checkpoint token layout (10 total tokens).

    NOTE: Must use vit_huge_patch16_224 (no init_values) NOT vit_huge_patch16_reg4_dinov2
    (init_values=1e-5). The dinov2 variant creates LayerScale modules (ls1.gamma, ls2.gamma)
    initialized to 1e-5, which the pretrained C-RADIOv4-H checkpoint does NOT have.
    LayerScale at 1e-5 makes each transformer block: x + 1e-5 * attn/norm, effectively
    bypassing all 32 blocks and producing constant embeddings regardless of input.
    See debug_v8_layerscale_bug.md for full analysis.
    """
    return RADIO(
        backbone="vit_huge_patch16_224",
        summary_idxs=[0, 1, 2],
