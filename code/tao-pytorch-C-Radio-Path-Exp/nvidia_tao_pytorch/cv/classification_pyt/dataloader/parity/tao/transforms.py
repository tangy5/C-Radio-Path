...
"""TAO parity dumps for Phase 3: Quad, generate_homography_grid, DeferImage."""
...
def _deterministic_image(C=3, H=32, W=48):
    """Create a reproducible float32 image tensor."""
    torch.manual_seed(0)
    return torch.rand(C, H, W)


def _standard_transforms(obj):
    """Apply a fixed sequence of transforms to a Quad or DeferImage.

    Returns a list of snapshots (bounds for Quad, stm for DeferImage) after
    each operation so parity can be checked step-by-step.
    """
    snapshots = {}
