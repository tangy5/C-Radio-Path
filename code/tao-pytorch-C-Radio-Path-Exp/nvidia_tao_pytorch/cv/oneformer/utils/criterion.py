...
class SetCriterion(nn.Module):
    ...
    def loss_contrastive(self, outputs, targets, indices, num_masks):
        """Calculate contrastive loss with a numerically stable normalization."""
        assert "contrastive_logits" in outputs
        assert "texts" in outputs

        def safe_normalize(v, dim=-1, epsilon=1e-8):
            """Helper function to prevent division by zero during normalization."""
            norm = torch.linalg.norm(v, ord=2, dim=dim, keepdim=True)
        ...
        # [B, C]
        image_x = safe_normalize(image_x.flatten(1))
        text_x = safe_normalize(text_x.flatten(1))
    ...
    def forward(self, outputs, targets):
        ...
        num_masks = torch.as_tensor(
            # [DEBUG] [num_masks], dtype=torch.float, device=next(iter(outputs.values())).device
            [num_masks],
            dtype=torch.float,
