...
CONFIG_ROOT = os.path.join(
    ROOT_DIR,"nvidia_tao_pytorch/cv/rtdetr/experiment_specs"
)
...
def _test_dataset_spec():
    dataset_config = RTDatasetConfig()
    yield dataset_config


@pytest.fixture
def _test_augmentation_spec():
    augmentation_config = RTAugmentationConfig()
    yield augmentation_config


@pytest.fixture
def _test_model_spec():
    model_config = RTModelConfig()
    yield model_config
...
TEST_CONFIG_BLOCKS = [
    (sample_model_config, RTModelConfig),
    (sample_dataset_config, RTDatasetConfig),
    (sample_train_config, RTTrainExpConfig),
