...
def _train_spec():
    ...
    experiment_config.distill = RTDistillationConfig()
    experiment_config.distill.teacher.backbone = "resnet_50"
    ...
    experiment_config.distill.bindings.append(DistillationBindingConfig())
    experiment_config.distill.bindings[0].student_module_name = "srcs"
    experiment_config.distill.bindings[0].teacher_module_name = "srcs"
    experiment_config.distill.bindings[0].criterion = 'IOU'
    experiment_config.distill.bindings[0].weight = 1.0

    experiment_config.model.backbone = "resnet_34"
...
@pytest.mark.parametrize("student_arch, student, teacher, criterion, weight", 
                        [("resnet_34","srcs", "srcs", "IOU", 1.0),
                         ("resnet_34","dsrcs", "dsrcs", "IOU", 1.0)])
def test_distiller_fit(_test_sample_json, _train_spec, student_arch, student, teacher, criterion, weight):
    ...
    pt_model = RtdetrDistiller(_train_spec)
