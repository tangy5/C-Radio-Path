...
class DistillationLoss(nn.Module):
    ...
    def forward(
        ...
        # fixed_npy = "<EDGEAI_ROOT>/users/<user>/workspace/tmp/random_array.npy"
        # if os.path.exists(fixed_npy):
        #     random_array = np.load(fixed_npy)
        #     batch_input = torch.from_numpy(random_array).to(batch_input.device)
